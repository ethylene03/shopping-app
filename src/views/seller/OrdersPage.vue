...
onMounted(() => {
  document.title = 'Shopping App: Product Orders'
  fetchOrders()
})
...
async function fetchOrders(nextPage?: number) {
  ...
  const response = await getOrders(
    { role: 'SELLER', page: nextPage?.toString() || '0' },
...
<template>
  <section ref="orderContainer" id="orders" class="section" @scroll="onScroll">
    ...
    <p class="lead text-center text-md-start">Manage orders of your products here.</p>
